...
const checkLogin = async (userName, res) => {
  const user = await require("../services/storeService").loginService(userName);
...
const authorizeToken = async (req, res, next) => {
  ...
  try {
    const data = jwt.verify(token, process.env.ACCESS_TOKEN_API_SHOP);
    if (data.owner) {
      const status = await checkLogin(data.owner, res);
      if (status == "2") {
        res.json(
